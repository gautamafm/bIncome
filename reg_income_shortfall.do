...
reg linc3  i.age i.educ (c.age c.age2)#i.educ2 i.year i.state lfamsize [pw=wt] ///
    if B10 < . & inrange(year, 1976, 1996) & inrange(age, 20, 65)
...
forval i = 1/3 {
    gen short`i' = -1 * min(2, max(-2, linc3 - predlinc`i')) ///
        if predlinc`i' < . & linc3 < . & inrange(year, 1976, 1996)
...
/******************************************************/
qui reg B10 avshort2 f9linc3 linc3 $controls ///
    if inrange(year, 1976, 1986) & hhtag2 [pw=wt], cluster(id)
